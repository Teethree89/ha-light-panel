...
function nativeCard(definition,style){style=style||{};return{id:'native-'+definition.id,kind:'native',nativeId:definition.id,parent:definition.parent||'',title:style.title||definition.title,summary:definition.summary,x:number(style.x,definition.x),y:number(style.y,definition.y),w:number(style.w,definition.w),h:number(style.h,definition.h),baseW:definition.w,baseH:definition.h,background:'#102131',accent:'#2dd4bf',textColor:'#ffffff',titleSize:18,valueSize:22,portrait:clone(style.portrait)}}
...
function buildDraft(config){sourceConfig=clone(config||{});var panel=sourceConfig.panel||{};var layout=panel.layout||{};draft.canvas=clone(layout.canvas||draft.canvas);if(!draft.canvas.preset)draft.canvas.preset='frameo';draft.canvas.responsive=draft.canvas.responsive||'auto';var styles=layout.roomCards||{},nativeStyles=layout.nativeCards||{};draft.cards=nativeDefaults.map(function(definition){return nativeCard(definition,nativeStyles[definition.id]||{})}).concat((panel.rooms||[]).map(function(r,i){return roomCard(r,i,styles[r.id]||{})})).concat((layout.cards||[]).map(normalizeCustom));selectedId=draft.cards[0]?draft.cards[0].id:'';render();save()}
function exported(){var out=clone(sourceConfig);out.panel=out.panel||{};var old=out.panel.layout||{};var roomStyles={},custom=[],nativeCards={};out.panel.rooms=draft.cards.filter(function(c){return c.kind==='room'}).map(function(c){var room=clone(c.base);room.id=c.id;room.label=c.title;room.temp=spec(c.entity,c.attribute);if(c.subEntity)room.humidity=spec(c.subEntity,c.subAttribute);else delete room.humidity;if(c.battery)room.battery=c.battery;else delete room.battery;roomStyles[c.id]={x:c.x,y:c.y,w:c.w,h:c.h,background:c.background,accent:c.accent,textColor:c.textColor,titleSize:number(c.titleSize,18),valueSize:number(c.valueSize,52),portrait:clone(c.portrait||{})};return room});draft.cards.filter(function(c){return c.kind==='native'}).forEach(function(c){nativeCards[c.nativeId]={title:c.title,x:c.x,y:c.y,w:c.w,h:c.h}});draft.cards.filter(function(c){return c.kind!=='room'&&c.kind!=='native'}).forEach(function(c){custom.push({id:c.id,type:c.kind,title:c.title,entity:c.entity,attribute:c.attribute,subEntity:c.subEntity,subAttribute:c.subAttribute,content:c.content,x:c.x,y:c.y,w:c.w,h:c.h,background:c.background,accent:c.accent,textColor:c.textColor,titleSize:number(c.titleSize,18),valueSize:number(c.valueSize,38),portrait:clone(c.portrait||{})})});out.panel.layout=Object.assign({},old,{version:2,canvas:clone(draft.canvas),nativeCards:nativeCards,roomCards:roomStyles,cards:custom});return out}

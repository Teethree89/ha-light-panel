...
// The original Frameo dashboard is native SVG rather than a collection of
// Lovelace cards. These are its addressable SVG regions. Keeping the geometry
// in config makes the visual builder faithfully load the complete panel while
// preserving its existing buttons, live values, and entity bindings.
const NATIVE_LAYOUT_DEFAULTS = [
  { id: 'topCard0', title: 'Blink system', x: 24, y: 20, w: 230, h: 112 },
  { id: 'topCard1', title: 'Current mode', x: 270, y: 20, w: 230, h: 112 },
  { id: 'topCard2', title: 'Comfort band', x: 516, y: 20, w: 230, h: 112 },
  { id: 'topCard3', title: 'Outside', x: 762, y: 20, w: 230, h: 112 },
  { id: 'topCard4', title: 'HA box', x: 1008, y: 20, w: 248, h: 112 },
  { id: 'roomsPanel', title: 'Rooms & thermostats', x: 24, y: 156, w: 776, h: 620 },
  { id: 'btnBalance', title: 'Balance rooms', parent: 'roomsPanel', x: 24, y: 532, w: 226, h: 70 },
  { id: 'btnAlarms', title: 'Smoke & CO alarms', parent: 'roomsPanel', x: 275, y: 532, w: 226, h: 70 },
  { id: 'btnSilence', title: 'Smoke alarm status', parent: 'roomsPanel', x: 526, y: 532, w: 226, h: 70 },
  { id: 'controls', title: 'Family target & controls', x: 824, y: 156, w: 432, h: 620 }
];

function configuredNativeLayoutCards() {
  const saved = isPlainObject(PANEL.layout?.nativeCards) ? PANEL.layout.nativeCards : {};
  return NATIVE_LAYOUT_DEFAULTS.map(definition => {
    const card = isPlainObject(saved[definition.id]) ? saved[definition.id] : {};
    return {
      ...definition,
      baseW: definition.w,
      baseH: definition.h,
      title: String(card.title || definition.title),
      x: layoutNumber(card.x, definition.x, 0, 1280),
      y: layoutNumber(card.y, definition.y, 0, 800),
      w: layoutNumber(card.w, definition.w, 34, 1400),
      h: layoutNumber(card.h, definition.h, 28, 1400)
    };
  });
}
...
function configuredLayoutCards() {
  ...
  return cards.slice(0, 30).map((card, index) => ({
    ...
    // `entity` was the v0.7.4 name. Keep it readable, but make new cards
    // explicit about whether they display information or perform an action.
    type: card.type === 'text' ? 'text' : card.type === 'action' ? 'action' : 'display',
    ...
    action: isPlainObject(card.action) ? {
      ...
      confirm: modalSettings(card.action.confirm, {
        title: String(card.title || 'Run action?'),
        body: 'Run this Home Assistant action now?',
        confirmLabel: 'Run action'
      }),
      success: modalSettings(card.action.success, {
        title: String(card.title || 'Action complete'),
        body: 'Home Assistant accepted the action.'
      }),
      failure: modalSettings(card.action.failure, {
        enabled: true,
        title: String(card.title || 'Action failed'),
        body: ''
      })
    } : null,
...
function layoutCardMarkup() {
  return configuredLayoutCards().map(card => `
    <g id="layoutCard-${escapeHtml(card.id)}" class="layout-card${card.type === 'action' ? ' button layout-action-card' : ''}"${card.type === 'action' ? ` data-layout-action-id="${escapeHtml(card.id)}" data-action="layout-${escapeHtml(card.id)}"` : ''} transform="translate(${card.x} ${card.y})">
      ...
      <text id="layoutValue-${escapeHtml(card.id)}" x="18" y="${Math.min(card.h - 10, Math.max(62, card.titleSize + card.valueSize + 18))}" fill="${card.textColor}" font-size="${card.valueSize}" font-weight="850">${card.type === 'action' ? 'Tap to run' : '--'}</text>
...
function modalSettings(value, fallback = {}) {
  const source = isPlainObject(value) ? value : {};
  return {
    enabled: source.enabled === undefined ? fallback.enabled === true : source.enabled === true,
    title: String(source.title || fallback.title || ''),
    body: String(source.body || fallback.body || ''),
    confirmLabel: String(source.confirmLabel || fallback.confirmLabel || 'Continue')
  };
}

// Balance Rooms started as a fixed script action. Keep that default compatible,
// but let the native SVG card own a safe service and modal policy in config.
function balanceActionConfig() {
  const configured = isPlainObject(PANEL.layout?.nativeCards?.btnBalance?.action)
    ? PANEL.layout.nativeCards.btnBalance.action
    : {};
  const legacy = isPlainObject((PANEL.actions || {}).assist) ? PANEL.actions.assist : {};
  return {
    service: String(configured.service || legacy.service || ''),
    data: isPlainObject(configured.data) ? configured.data : (isPlainObject(legacy.data) ? legacy.data : {}),
    requireReady: configured.requireReady !== false,
    confirm: modalSettings(configured.confirm, {
      title: 'Balance Rooms?',
      body: 'Run the configured airflow-balance action now?',
      confirmLabel: 'Run balance'
    }),
    success: modalSettings(configured.success, {
      title: 'Balance Rooms',
      body: 'The balance action was sent to Home Assistant.'
    }),
    failure: modalSettings(configured.failure, {
      enabled: true,
      title: 'Balance Rooms unavailable',
      body: ''
    })
  };
}

// Generic action cards deliberately resolve their id on the server. The
// browser may choose a configured card, but it cannot substitute a different
// service or payload into the request.
function layoutActionConfig(id) {
  const card = configuredLayoutCards().find(item => item.type === 'action' && item.id === String(id || ''));
  if (!card || !card.action?.service) return null;
  return card.action;
}
...
async function callAction(name, options = {}) {
  ...
  if (name === 'assist') {
    const action = balanceActionConfig();
    if (action.requireReady) {
      const availability = balanceAvailability();
      if (!availability.canRun) {
        const error = new Error(`${availability.reason} ${availability.detail}.`);
        error.statusCode = 409;
        throw error;
      }
    }
    if (!action.service) throw new Error('No Balance Rooms action is configured.');
    await callConfiguredService(action);
    return;
  }

  if (name === 'layoutCard') {
    const action = layoutActionConfig(options.id);
    if (!action) throw new Error('This action card is not configured.');
    await callConfiguredService(action);
    return;
  }

  if (name === 'reset' || name === 'silenceAlarm') {
...
<head>
  ...
  <style>
    ...
    .secondary-modal { background: #334155; color: #f8fafc; }
...
${layoutCardMarkup()}
  ...
  <div id="infoModal" class="modal-backdrop hidden" role="dialog" aria-modal="true" aria-labelledby="infoModalTitle">
    <div class="modal">
      ...
      <div class="modal-actions"><button id="infoModalClose" class="modal-button secondary-modal" type="button">Got it</button><button id="infoModalConfirm" class="modal-button" type="button" hidden>Continue</button></div>
    </div>
  </div>
  ...
  <script>
    const els = {
      ...
      infoModalConfirm: document.getElementById('infoModalConfirm'),
    ...
    const layoutActionCards = layoutCardsConfig.filter(card => card.type === 'action' && card.action);
    const nativeLayoutConfig = ${jsValue(configuredNativeLayoutCards())};
    const balanceActionConfig = ${jsValue(balanceActionConfig())};
    ...
    // Native regions retain their internal SVG and behavior; the builder only
    // changes the region's outer box. Scaling the group carries its live text,
    // controls, and child cards with it instead of replacing them with a mock.
    function applyConfiguredNativeLayout() {
      if (portraitMedia && portraitMedia.matches) return;
      const titleTargets = {
        topCard0: 'hdrBlink', topCard1: 'hdrMode', topCard2: 'hdrComfort',
        topCard3: 'hdrOutside', topCard4: 'hdrHaBox', roomsPanel: 'roomsTitleLabel',
        btnBalance: 'btnBalanceText', controls: 'hdrFamily'
      };
      nativeLayoutConfig.forEach(card => {
        const node = byId(card.id);
        if (!node) return;
        const scaleX = card.w / card.baseW;
        const scaleY = card.h / card.baseH;
        if (![card.x, card.y, scaleX, scaleY].every(Number.isFinite)) return;
        node.setAttribute('transform', 'translate(' + card.x + ' ' + card.y + ') scale(' + scaleX + ' ' + scaleY + ')');
        const title = byId(titleTargets[card.id]);
    ...
    function showModal(title, body, isHtml = false, options = {}) {
      ...
      if (els.infoModalClose) els.infoModalClose.textContent = options.closeLabel || 'Got it';
      if (els.infoModalConfirm) {
        els.infoModalConfirm.hidden = true;
        els.infoModalConfirm.onclick = null;
      }
      if (els.infoModal) els.infoModal.classList.remove('hidden');
    }

    function showConfirmModal(title, body, confirmLabel, onConfirm) {
      showModal(title, body, false, { closeLabel: 'Cancel' });
      if (!els.infoModalConfirm) return;
      els.infoModalConfirm.hidden = false;
      els.infoModalConfirm.textContent = confirmLabel || 'Continue';
      els.infoModalConfirm.onclick = () => {
        hideModal();
        onConfirm();
      };
    }
    ...
    function balanceModalCopy(kind, fallback) {
      const modal = balanceActionConfig[kind] || {};
      return {
        enabled: modal.enabled === true,
        title: modal.title || fallback.title,
        body: modal.body || fallback.body,
        confirmLabel: modal.confirmLabel || fallback.confirmLabel || 'Continue'
      };
    }

    function layoutActionModalCopy(id, kind, fallback) {
      const card = layoutActionCards.find(item => item.id === id);
      const modal = card?.action?.[kind] || {};
      return {
        enabled: modal.enabled === true,
        title: modal.title || fallback.title,
        body: modal.body || fallback.body,
    ...
    async function action(name, options = {}, confirmed = false) {
      if (name === 'showAlarms') {
        showAlarmModal(appState.latest && appState.latest.alarmPanel);
        return;
      }

      if (name === 'assist' && balanceActionConfig.requireReady && appState.latest?.balance && !appState.latest.balance.canRun) {
        const balance = appState.latest.balance;
        const failure = balanceModalCopy('failure', { title: 'Balance Rooms unavailable', body: balance.reason + ' ' + balance.detail + '.' });
        if (failure.enabled) showModal(failure.title, failure.body);
        return;
      }

      if (name === 'assist' && !confirmed) {
        const confirm = balanceModalCopy('confirm', { title: 'Balance Rooms?', body: 'Run the configured airflow-balance action now?', confirmLabel: 'Run balance' });
        if (confirm.enabled) {
          showConfirmModal(confirm.title, confirm.body, confirm.confirmLabel, () => action('assist', options, true));
          return;
        }
      }

      if (name === 'layoutCard' && !confirmed) {
        const confirm = layoutActionModalCopy(options.id, 'confirm', {
          title: 'Run action?', body: 'Run this Home Assistant action now?', confirmLabel: 'Run action'
        });
        if (confirm.enabled) {
          showConfirmModal(confirm.title, confirm.body, confirm.confirmLabel, () => action('layoutCard', options, true));
          return;
        }
      }

      els.connection.textContent = 'sending';
      setPending(name === 'layoutCard' ? 'layout-' + options.id : name, true);
      try {
        ...
        await refresh();
        if (name === 'assist') {
          const success = balanceModalCopy('success', { title: 'Balance Rooms', body: 'The balance action was sent to Home Assistant.' });
          if (success.enabled) showModal(success.title, success.body);
        }
        if (name === 'layoutCard') {
          const success = layoutActionModalCopy(options.id, 'success', {
            title: 'Action complete', body: 'Home Assistant accepted the action.'
          });
          if (success.enabled) showModal(success.title, success.body);
        }
      } catch (error) {
        ...
        if (name === 'assist') {
          const failure = balanceModalCopy('failure', { title: 'Balance Rooms failed', body: error.message || 'Home Assistant could not run the balance action.' });
          if (failure.enabled) showModal(failure.title, failure.body);
        }
        if (name === 'layoutCard') {
          const failure = layoutActionModalCopy(options.id, 'failure', {
            title: 'Action failed', body: error.message || 'Home Assistant could not run the action.'
          });
          if (failure.enabled) showModal(failure.title, failure.body || error.message || 'Home Assistant could not run the action.');
        }
      } finally {
        setPending(name === 'layoutCard' ? 'layout-' + options.id : name, false);
      }
    }
    ...
    document.querySelectorAll('[data-action]').forEach(node => {
      node.addEventListener('pointerup', event => {
        ...
        action(name.startsWith('layout-') ? 'layoutCard' : name, {
          id: name.startsWith('layout-') ? node.dataset.layoutActionId : undefined,
...
</style><style>.check{display:flex;align-items:center;gap:8px;margin:9px 0;color:var(--text)}.check input{width:auto;accent-color:var(--mint)}textarea{min-height:70px;resize:vertical}</style></head><body>
<header><div class="brand">HA Light Panel <small>Layout Builder</small></div><div class="saved" id="saved">Loading your complete SVG panel…</div><div class="header-actions"><button class="secondary" id="reload">Reset to saved panel</button><button class="secondary" id="copy">Copy config</button><button id="download">Download config</button></div></header>
<main><aside><h2>Add to panel</h2><div class="type-list" id="types"></div><div class="help"><strong>Generic SVG card workspace.</strong><br>Display cards can bind any Home Assistant entity or attribute. Action buttons call configured services or scripts and can show confirmation, success, and failure modals. Imported SVG regions remain editable without defining what new cards must be.</div></aside><section class="workspace"><div class="workspace-title"><div><h1>Design your panel</h1><p>Imported regions load first. Add general-purpose display, action, or text cards anywhere on the canvas.</p></div><div class="toolbar"><select id="preset" aria-label="Canvas size"></select><div class="mode-toggle"><button class="active" data-mode="landscape">Landscape</button><button data-mode="portrait">Portrait</button></div></div></div><div class="canvas-frame"><div id="canvas" class="canvas"></div></div></section><aside class="right"><h2>Inspector</h2><div id="inspector"></div></aside></main><datalist id="entityChoices"></datalist>
<script>(function(){
...
var types=[{type:'display',label:'Display card',hint:'Show any entity, attribute, and optional secondary value'},{type:'action',label:'Action button',hint:'Run a Home Assistant service or script with optional modals'},{type:'text',label:'Text card',hint:'A title or fixed message in your panel style'}];
var nativeDefaults=[{id:'topCard0',title:'Blink system',summary:'Alarm state and arm control',x:24,y:20,w:230,h:112},{id:'topCard1',title:'Current mode',summary:'Live HVAC operating mode',x:270,y:20,w:230,h:112},{id:'topCard2',title:'Comfort band',summary:'Heat and cool targets',x:516,y:20,w:230,h:112},{id:'topCard3',title:'Outside',summary:'Outdoor temperature',x:762,y:20,w:230,h:112},{id:'topCard4',title:'HA box',summary:'Home Assistant host health',x:1008,y:20,w:248,h:112},{id:'roomsPanel',title:'Rooms & thermostats',summary:'Room cards and safety actions',x:24,y:156,w:776,h:620},{id:'btnBalance',title:'Balance rooms',summary:'Airflow-assist action',parent:'roomsPanel',x:24,y:532,w:226,h:70},{id:'btnAlarms',title:'Smoke & CO alarms',summary:'Safety status action',parent:'roomsPanel',x:275,y:532,w:226,h:70},{id:'btnSilence',title:'Smoke alarm status',summary:'Silence active alarms',parent:'roomsPanel',x:526,y:532,w:226,h:70},{id:'controls',title:'Family target & controls',summary:'Comfort, status, cameras, settings',x:824,y:156,w:432,h:620}];
...
function actionDataText(value){try{return JSON.stringify(value&&typeof value==='object'?value:{},null,2)}catch(_){return '{}'}}function nativeCard(definition,style,fallbackAction){style=style||{};var action=style.action||fallbackAction||{},confirm=action.confirm||{},success=action.success||{},failure=action.failure||{};return{id:'native-'+definition.id,kind:'native',nativeId:definition.id,parent:definition.parent||'',title:style.title||definition.title,summary:definition.summary,x:number(style.x,definition.x),y:number(style.y,definition.y),w:number(style.w,definition.w),h:number(style.h,definition.h),baseW:definition.w,baseH:definition.h,background:'#102131',accent:'#2dd4bf',textColor:'#ffffff',titleSize:18,valueSize:22,portrait:clone(style.portrait),actionService:action.service||'',actionData:actionDataText(action.data),actionRequireReady:action.requireReady!==false,actionConfirmEnabled:confirm.enabled===true,actionConfirmTitle:confirm.title||'Balance Rooms?',actionConfirmBody:confirm.body||'Run the configured airflow-balance action now?',actionConfirmLabel:confirm.confirmLabel||'Run balance',actionSuccessEnabled:success.enabled===true,actionSuccessTitle:success.title||'Balance Rooms',actionSuccessBody:success.body||'The balance action was sent to Home Assistant.',actionFailureEnabled:failure.enabled!==false,actionFailureTitle:failure.title||'Balance Rooms unavailable',actionFailureBody:failure.body||''}}
function roomCard(room,index,style){var temp=specParts(room.temp);var hum=specParts(room.humidity);style=style||{};var pos=[[24,94],[275,94],[526,94],[24,313],[275,313],[526,313]][index]||[24+(index%3)*251,94+Math.floor(index/3)*219];return{id:room.id||uid(),kind:'room',parent:'roomsPanel',title:room.label||'Room',entity:temp.entity,attribute:temp.attribute,subEntity:hum.entity,subAttribute:hum.attribute,battery:specParts(room.battery).entity,x:number(style.x,pos[0]),y:number(style.y,pos[1]),w:number(style.w,226),h:number(style.h,208),background:style.background||'#102131',accent:style.accent||'#22c55e',textColor:style.textColor||'#ffffff',titleSize:number(style.titleSize,18),valueSize:number(style.valueSize,52),base:clone(room),portrait:clone(style.portrait)}}
function normalizeAction(value,title){var action=value&&typeof value==='object'?value:{},confirm=action.confirm||{},success=action.success||{},failure=action.failure||{};return{service:action.service||'',data:actionDataText(action.data),confirmEnabled:confirm.enabled===true,confirmTitle:confirm.title||title||'Run action?',confirmBody:confirm.body||'Run this Home Assistant action now?',confirmLabel:confirm.confirmLabel||'Run action',successEnabled:success.enabled===true,successTitle:success.title||title||'Action complete',successBody:success.body||'Home Assistant accepted the action.',failureEnabled:failure.enabled!==false,failureTitle:failure.title||title||'Action failed',failureBody:failure.body||''}}
function normalizeCustom(card,index){var pos=[780,90+index*155],kind=card.type==='text'?'text':card.type==='action'?'action':'display';return{id:card.id||uid(),kind:kind,title:card.title||'New card',entity:card.entity||'',attribute:card.attribute||'',subEntity:card.subEntity||'',subAttribute:card.subAttribute||'',content:card.content||'',action:normalizeAction(card.action,card.title||'Run action'),x:number(card.x,pos[0]),y:number(card.y,pos[1]),w:number(card.w,220),h:number(card.h,132),background:card.background||'#102131',accent:card.accent||(kind==='action'?'#2dd4bf':'#38bdf8'),textColor:card.textColor||'#ffffff',titleSize:number(card.titleSize,18),valueSize:number(card.valueSize,38),portrait:clone(card.portrait)}}
function newCard(kind){var i=draft.cards.length,isAction=kind==='action',isText=kind==='text',title=isAction?'Run action':isText?'Panel note':'Entity value',card={id:uid(),kind:kind,parent:'',title:title,entity:'',attribute:'',subEntity:'',subAttribute:'',content:isText?'Add your message':isAction?'Tap to run':'',action:normalizeAction({},title),x:800,y:90+(i%4)*145,w:220,h:isAction?92:132,background:'#102131',accent:isText?'#a78bfa':isAction?'#2dd4bf':'#38bdf8',textColor:'#ffffff',titleSize:18,valueSize:isText?24:isAction?20:42,portrait:{}};draft.cards.push(card);selectedId=card.id;commit()}
function selected(){return draft.cards.find(function(c){return c.id===selectedId})}function activeBox(card){if(mode==='portrait'&&card.portrait&&card.portrait.x!==undefined)return card.portrait;return card}function parentCard(card){return card.parent?draft.cards.find(function(c){return c.kind==='native'&&c.nativeId===card.parent}):null}function displayBox(card){var box=activeBox(card),parent=mode==='portrait'?null:parentCard(card);if(!parent)return box;var parentBox=activeBox(parent),sx=parentBox.w/parent.baseW,sy=parentBox.h/parent.baseH;return{x:parentBox.x+box.x*sx,y:parentBox.y+box.y*sy,w:box.w*sx,h:box.h*sy}}function ensurePortrait(card){if(!card.portrait||card.portrait.x===undefined)card.portrait={x:32,y:42+draft.cards.indexOf(card)*110,w:Math.max(120,draft.canvas.w-64),h:Math.min(card.h,96)};return card.portrait}function setBox(card,key,value){var box=mode==='portrait'&&draft.canvas.responsive==='custom'?ensurePortrait(card):card;box[key]=Math.round(number(value,box[key]))}
function renderTypes(){$('types').innerHTML=types.map(function(t){return '<button class="type" data-type="'+t.type+'">'+t.label+'<span>'+t.hint+'</span></button>'}).join('')}function values(card){if(card.kind==='text')return card.content||'Add a message';if(card.kind==='action')return card.content||'Tap to run';if(card.kind==='room')return card.entity?'72°':'--';if(card.kind==='native')return '';return card.entity?'72':'--'}function cardHtml(card){var b=displayBox(card),is=card.id===selectedId,isNative=card.kind==='native',sub=isNative?card.summary:(card.kind==='room'?(card.subEntity?'Humidity · '+card.subEntity:'Add humidity binding'):card.kind==='action'?(card.action&&card.action.service?card.action.service:'Choose a service or script'):card.kind==='display'?(card.subEntity||card.attribute||'Home Assistant entity'):'Fixed panel text'),kind=isNative?'Live SVG region':card.kind==='room'?'Imported room':card.kind==='action'?'Action button':card.kind==='display'?'Display card':'Text',layer=card.parent?2:(isNative?1:3);var style='left:'+(b.x/draft.canvas.w*100)+'%;top:'+(b.y/draft.canvas.h*100)+'%;width:'+(b.w/draft.canvas.w*100)+'%;height:'+(b.h/draft.canvas.h*100)+'%;z-index:'+layer+';--card-bg:'+esc(isNative&&card.nativeId==='roomsPanel'?'#071017':card.background)+';--accent:'+esc(card.accent)+';--card-text:'+esc(card.textColor)+';--title-size:'+number(card.titleSize,18)+'px;--value-size:'+number(card.valueSize,38)+'px';return '<article class="card '+(is?'selected ':'')+(isNative?'native-region':'')+(card.kind==='action'?' action-preview':'')+'" data-id="'+esc(card.id)+'" style="'+style+'"><div class="native-card"><div class="accent"></div><div class="card-kind">'+esc(kind)+'</div><div class="card-title">'+esc(card.title||'Untitled')+'</div>'+(isNative?'':'<div class="card-value">'+esc(values(card))+'</div>')+'<div class="card-sub">'+esc(sub)+'</div></div><div class="card-tools">'+(isNative?'':'<button data-move="up" title="Move earlier">↑</button><button data-move="down" title="Move later">↓</button><button data-remove="yes" title="Delete">×</button>')+'</div><i class="resize" data-resize="yes" title="Resize region"></i></article>'}
function renderCanvas(){var preset=presets.find(function(p){return p.id===draft.canvas.preset})||presets[0];var w=mode==='portrait'&&preset.w>preset.h?preset.h:preset.w,h=mode==='portrait'&&preset.w>preset.h?preset.w:preset.h;draft.canvas.w=w;draft.canvas.h=h;var canvas=$('canvas');canvas.style.width='min(100%, '+w+'px)';canvas.style.aspectRatio=w+' / '+h;canvas.innerHTML=draft.cards.length?draft.cards.map(cardHtml).join(''):'<div class="empty"><b>Your panel canvas is ready.</b>Add a display card, action button, or text card from the left.</div>'}
function field(label,key,value,kind){var input;if(kind==='select')input='<select data-field="'+key+'"><option value="auto"'+(value==='auto'?' selected':'')+'>Automatic stack</option><option value="custom"'+(value==='custom'?' selected':'')+'>Custom portrait layout</option></select>';else if(kind==='textarea')input='<textarea data-field="'+key+'">'+esc(value)+'</textarea>';else input='<input data-field="'+key+'" '+(key.indexOf('Entity')>0||key==='entity'?'list="entityChoices"':'')+' value="'+esc(value)+'" '+(kind==='color'?'type="color"':'')+'>';return '<div class="field"><label>'+label+input+'</label></div>'}function check(label,key,value){return '<label class="check"><input type="checkbox" data-field="'+key+'"'+(value?' checked':'')+'><span>'+label+'</span></label>'}function fieldsRow(a,b){return '<div class="field-row">'+a+b+'</div>'}function actionFields(action){action=action||normalizeAction({},'Run action');return '<div class="section">Action</div>'+field('Home Assistant service or script','actionService',action.service)+field('Service data (JSON)','actionData',action.data,'textarea')+'<div class="section">Confirmation modal</div>'+check('Ask before running','actionConfirmEnabled',action.confirmEnabled)+field('Title','actionConfirmTitle',action.confirmTitle)+field('Message','actionConfirmBody',action.confirmBody,'textarea')+field('Confirm button label','actionConfirmLabel',action.confirmLabel)+'<div class="section">Success modal</div>'+check('Show after Home Assistant accepts the action','actionSuccessEnabled',action.successEnabled)+field('Title','actionSuccessTitle',action.successTitle)+field('Message','actionSuccessBody',action.successBody,'textarea')+'<div class="section">Failure modal</div>'+check('Show if Home Assistant returns an error','actionFailureEnabled',action.failureEnabled)+field('Title','actionFailureTitle',action.failureTitle)+field('Message (leave blank for the real error)','actionFailureBody',action.failureBody,'textarea')}
function renderInspector(){var card=selected();if(!card){$('inspector').innerHTML='<div class="inspector-empty">Select a live region, imported card, or new generic card to edit it.</div>';return}var box=activeBox(card),isNative=card.kind==='native',html=field(isNative?'Region title':'Title','title',card.title);if(isNative){html+='<p class="tiny">This imported SVG region keeps its existing controls and values. New generic cards are independent of this HVAC layout.</p>';if(card.nativeId==='btnBalance'){html+=actionFields({service:card.actionService,data:card.actionData,confirmEnabled:card.actionConfirmEnabled,confirmTitle:card.actionConfirmTitle,confirmBody:card.actionConfirmBody,confirmLabel:card.actionConfirmLabel,successEnabled:card.actionSuccessEnabled,successTitle:card.actionSuccessTitle,successBody:card.actionSuccessBody,failureEnabled:card.actionFailureEnabled,failureTitle:card.actionFailureTitle,failureBody:card.actionFailureBody})}}else{if(card.kind==='display'||card.kind==='action'||card.kind==='room')html+=field(card.kind==='action'?'Status entity (optional)':'Primary entity','entity',card.entity)+field('Primary attribute (optional)','attribute',card.attribute);if(card.kind==='display')html+=field('Secondary entity / caption (optional)','subEntity',card.subEntity)+field('Secondary attribute (optional)','subAttribute',card.subAttribute);if(card.kind==='room')html+=field('Humidity entity (optional)','subEntity',card.subEntity)+field('Humidity attribute (optional)','subAttribute',card.subAttribute)+field('Battery entity (optional)','battery',card.battery);if(card.kind==='action')html+=field('Button caption (optional)','content',card.content)+actionFields(card.action);if(card.kind==='text')html+=field('Text content','content',card.content);html+='<div class="section">Card style</div>'+fieldsRow(field('Background','background',card.background,'color'),field('Accent','accent',card.accent,'color'))+field('Text color','textColor',card.textColor,'color')+fieldsRow(field('Title font px','titleSize',card.titleSize),field('Value font px','valueSize',card.valueSize))}html+='<div class="section">'+(mode==='portrait'?'Portrait ':'Landscape ')+'geometry</div>'+fieldsRow(field('X','x',box.x),field('Y','y',box.y))+fieldsRow(field('Width','w',box.w),field('Height','h',box.h));if(mode==='portrait'&&!isNative)html+='<div class="section">Responsive behavior</div>'+field('Portrait layout','responsive',draft.canvas.responsive,'select')+'<p class="tiny">Automatic stacks cards for a useful default. Choose Custom portrait layout to drag and resize this card separately for portrait.</p>';if(!isNative)html+='<div class="card-actions"><button class="secondary" id="duplicate">Duplicate</button><button class="danger" id="delete">Delete</button></div>';html+='<p class="tiny">Downloaded config is explicit and safe: install it as <code>config.json</code>, then restart the panel service to apply it.</p>';$('inspector').innerHTML=html}
...
function buildDraft(config){sourceConfig=clone(config||{});var panel=sourceConfig.panel||{};var layout=panel.layout||{},fallbackAction=(panel.actions||{}).assist||{};draft.canvas=clone(layout.canvas||draft.canvas);if(!draft.canvas.preset)draft.canvas.preset='frameo';draft.canvas.responsive=draft.canvas.responsive||'auto';var styles=layout.roomCards||{},nativeStyles=layout.nativeCards||{};draft.cards=nativeDefaults.map(function(definition){return nativeCard(definition,nativeStyles[definition.id]||{},definition.id==='btnBalance'?fallbackAction:{})}).concat((panel.rooms||[]).map(function(r,i){return roomCard(r,i,styles[r.id]||{})})).concat((layout.cards||[]).map(normalizeCustom));selectedId=draft.cards[0]?draft.cards[0].id:'';render();save()}
function actionData(value){try{var parsed=JSON.parse(value||'{}');return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:{}}catch(_){return {}}}function exportAction(action){action=action||normalizeAction({},'Run action');return{service:action.service||'',data:actionData(action.data),confirm:{enabled:action.confirmEnabled===true,title:action.confirmTitle,body:action.confirmBody,confirmLabel:action.confirmLabel},success:{enabled:action.successEnabled===true,title:action.successTitle,body:action.successBody},failure:{enabled:action.failureEnabled!==false,title:action.failureTitle,body:action.failureBody}}}function exported(){var out=clone(sourceConfig);out.panel=out.panel||{};var old=out.panel.layout||{},roomStyles={},custom=[],nativeCards={};out.panel.rooms=draft.cards.filter(function(c){return c.kind==='room'}).map(function(c){var room=clone(c.base);room.id=c.id;room.label=c.title;room.temp=spec(c.entity,c.attribute);if(c.subEntity)room.humidity=spec(c.subEntity,c.subAttribute);else delete room.humidity;if(c.battery)room.battery=c.battery;else delete room.battery;roomStyles[c.id]={x:c.x,y:c.y,w:c.w,h:c.h,background:c.background,accent:c.accent,textColor:c.textColor,titleSize:number(c.titleSize,18),valueSize:number(c.valueSize,52),portrait:clone(c.portrait||{})};return room});draft.cards.filter(function(c){return c.kind==='native'}).forEach(function(c){var saved={title:c.title,x:c.x,y:c.y,w:c.w,h:c.h};if(c.nativeId==='btnBalance')saved.action={service:c.actionService,data:actionData(c.actionData),requireReady:c.actionRequireReady!==false,confirm:{enabled:c.actionConfirmEnabled===true,title:c.actionConfirmTitle,body:c.actionConfirmBody,confirmLabel:c.actionConfirmLabel},success:{enabled:c.actionSuccessEnabled===true,title:c.actionSuccessTitle,body:c.actionSuccessBody},failure:{enabled:c.actionFailureEnabled!==false,title:c.actionFailureTitle,body:c.actionFailureBody}};nativeCards[c.nativeId]=saved});draft.cards.filter(function(c){return c.kind!=='room'&&c.kind!=='native'}).forEach(function(c){custom.push({id:c.id,type:c.kind,title:c.title,entity:c.entity,attribute:c.attribute,subEntity:c.subEntity,subAttribute:c.subAttribute,content:c.content,action:c.kind==='action'?exportAction(c.action):undefined,x:c.x,y:c.y,w:c.w,h:c.h,background:c.background,accent:c.accent,textColor:c.textColor,titleSize:number(c.titleSize,18),valueSize:number(c.valueSize,38),portrait:clone(c.portrait||{})})});out.panel.layout=Object.assign({},old,{version:4,canvas:clone(draft.canvas),nativeCards:nativeCards,roomCards:roomStyles,cards:custom});return out}
...
$('types').addEventListener('click',function(e){var t=e.target.closest('[data-type]');if(t)newCard(t.dataset.type)});$('canvas').addEventListener('click',function(e){var node=e.target.closest('.card');if(!node)return;var card=draft.cards.find(function(c){return c.id===node.dataset.id});if(!card)return;var action=e.target.closest('[data-remove],[data-move]');var index=draft.cards.indexOf(card);if(action&&action.dataset.remove&&card.kind!=='native'){draft.cards.splice(index,1);selectedId=draft.cards[0]?draft.cards[0].id:'';commit();return}if(action&&action.dataset.move&&card.kind!=='native'){var target=action.dataset.move==='up'?index-1:index+1;if(target>=0&&target<draft.cards.length){draft.cards.splice(index,1);draft.cards.splice(target,0,card);commit()}return}selectedId=card.id;render()});$('canvas').addEventListener('pointerdown',function(e){var node=e.target.closest('.card');if(!node||e.target.closest('[data-move],[data-remove]'))return;var card=draft.cards.find(function(c){return c.id===node.dataset.id});if(!card)return;selectedId=card.id;var b=activeBox(card),parent=parentCard(card),rect=$('canvas').getBoundingClientRect(),parentX=parent?parent.w/parent.baseW:1,parentY=parent?parent.h/parent.baseH:1;drag={card:card,resize:Boolean(e.target.closest('[data-resize]')),startX:e.clientX,startY:e.clientY,x:b.x,y:b.y,w:b.w,h:b.h,scaleX:draft.canvas.w/rect.width/parentX,scaleY:draft.canvas.h/rect.height/parentY};node.setPointerCapture(e.pointerId);e.preventDefault()});$('canvas').addEventListener('pointermove',function(e){if(!drag)return;var dx=(e.clientX-drag.startX)*drag.scaleX,dy=(e.clientY-drag.startY)*drag.scaleY;if(mode==='portrait'&&draft.canvas.responsive==='auto'&&drag.card.kind!=='native')draft.canvas.responsive='custom';var box=mode==='portrait'?ensurePortrait(drag.card):drag.card;if(drag.resize){box.w=Math.max(70,Math.round(drag.w+dx));box.h=Math.max(48,Math.round(drag.h+dy))}else{box.x=Math.max(0,Math.round(drag.x+dx));box.y=Math.max(0,Math.round(drag.y+dy))}renderCanvas();renderInspector()});$('canvas').addEventListener('pointerup',function(){if(drag){drag=null;save()}});$('inspector').addEventListener('input',function(e){var card=selected(),key=e.target.dataset.field,value=e.target.type==='checkbox'?e.target.checked:e.target.value;if(!card||!key)return;if(key==='responsive'){draft.canvas.responsive=value;if(value==='custom')ensurePortrait(card)}else if(['x','y','w','h'].indexOf(key)>=0)setBox(card,key,value);else if(card.kind==='action'&&key.indexOf('action')===0){var map={actionService:'service',actionData:'data',actionConfirmEnabled:'confirmEnabled',actionConfirmTitle:'confirmTitle',actionConfirmBody:'confirmBody',actionConfirmLabel:'confirmLabel',actionSuccessEnabled:'successEnabled',actionSuccessTitle:'successTitle',actionSuccessBody:'successBody',actionFailureEnabled:'failureEnabled',actionFailureTitle:'failureTitle',actionFailureBody:'failureBody'};card.action=card.action||normalizeAction({},card.title);card.action[map[key]]=value}else card[key]=value;commit()});$('inspector').addEventListener('click',function(e){var card=selected();if(!card||card.kind==='native')return;if(e.target.id==='delete'){draft.cards=draft.cards.filter(function(c){return c.id!==card.id});selectedId=draft.cards[0]?draft.cards[0].id:'';commit()}if(e.target.id==='duplicate'){var next=clone(card);next.id=uid();next.parent='';next.x+=18;next.y+=18;draft.cards.splice(draft.cards.indexOf(card)+1,0,next);selectedId=next.id;commit()}});$('preset').onchange=function(){draft.canvas.preset=this.value;commit()};document.querySelectorAll('[data-mode]').forEach(function(b){b.onclick=function(){mode=b.dataset.mode;document.querySelectorAll('[data-mode]').forEach(function(x){x.classList.toggle('active',x===b)});render()}});$('reload').onclick=function(){loadRemote()};$('copy').onclick=function(){copyText(JSON.stringify(exported(),null,2)+'\n').then(function(){$('saved').textContent='Panel config copied to clipboard'})};$('download').onclick=function(){var blob=new Blob([JSON.stringify(exported(),null,2)+'\n'],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ha-light-panel.config.json';a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},0);$('saved').textContent='Panel config downloaded'};renderTypes();renderPreset();render();loadRemote();loadEntities();
})();</script></body></html>`;
}

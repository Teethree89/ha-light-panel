...
class HaLightPanelSidebar extends HTMLElement {
  ...
  _overviewBody() {
    ...
    const updateRow = update.available
      ? `<button type="button" data-update ${this._busy ? "disabled" : ""}>Update panel</button><small>${this._escape(update.service || "Managed systemd updater ready")}</small>`
      : `<small>${this._escape(update.reason || "Use the add-on store, Docker, or the systemd bootstrap installer to update this panel.")}</small>`;
    return `<div class="overview"><div class="overview-card hero"><div><span class="pill ${live ? "good" : "bad"}">${live ? "Online" : "Needs attention"}</span><h2>HA Light Panel</h2><p>${this._escape(live ? "The panel service is reachable through Home Assistant." : status.detail || this._message)}</p></div><a href="${this._target()}" target="_blank" rel="noopener">Open dashboard ↗</a></div><div class="overview-grid"><div class="overview-card"><h3>Service</h3><strong>${this._escape(service)}</strong><small>Version ${this._escape(version)} · last poll ${this._escape(updated)}</small></div><div class="overview-card"><h3>Updates</h3>${updateRow}</div><div class="overview-card"><h3>Layout Builder</h3><p>Edit native SVG cards, entity bindings, font sizes, and responsive layout presets.</p><button type="button" data-tab="builder">Open builder</button></div><div class="overview-card"><h3>Other devices</h3><p>Copy the Home Assistant-backed links for tablets, phones, and desktops.</p><button type="button" class="secondary" data-tab="links">Device links</button></div></div></div>`;
  }

  async _updatePanel() {
    if (!this._hass || this._busy) return;
    this._busy = true; this._message = "Starting the managed panel update…"; this._render();
    try { const result = await this._hass.callApi("POST", "ha_light_panel/sidebar-update", {}); this._message = result.message || "Update started. The panel may reconnect shortly."; }
    catch (error) { this._message = (error && error.body) || "The panel update could not be started."; }
    finally { this._busy = false; window.setTimeout(() => this._checkPanel(), 1800); }
  }

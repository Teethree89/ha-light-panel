...
<main><aside><h2>Add to panel</h2><div class="type-list" id="types"></div><div class="help"><strong>Your complete native SVG workspace.</strong><br>Every live panel region loads onto this canvas. Drag or resize a region, room, or added card, then download the edited <code>config.json</code>. Nothing changes on the live panel until you install that file and restart the service.</div></aside><section class="workspace"><div class="workspace-title"><div><h1>Design your panel</h1><p>Live SVG regions, room cards, and action tiles load first. Add bound entity cards or text to extend them.</p></div><div class="toolbar"><select id="preset" aria-label="Canvas size"></select><div class="mode-toggle"><button class="active" data-mode="landscape">Landscape</button><button data-mode="portrait">Portrait</button></div></div></div><div class="canvas-frame"><div id="canvas" class="canvas"></div></div></section><aside class="right"><h2>Inspector</h2><div id="inspector"></div></aside></main><datalist id="entityChoices"></datalist>
<script>(function(){
...
function newCard(kind){var i=draft.cards.length;var card={id:uid(),kind:kind,parent:'',title:kind==='room'?'New room':kind==='text'?'Panel note':'Entity value',entity:'',attribute:'',subEntity:'',subAttribute:'',content:kind==='text'?'Add your message':'',battery:'',x:800,y:90+(i%4)*145,w:220,h:132,background:'#102131',accent:kind==='text'?'#a78bfa':'#38bdf8',textColor:'#ffffff',titleSize:18,valueSize:kind==='text'?24:42,portrait:{}};if(kind==='room'){card.parent='roomsPanel';card.w=226;card.h=208;card.valueSize=52}draft.cards.push(card);selectedId=card.id;commit()}
